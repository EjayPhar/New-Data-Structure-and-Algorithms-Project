...
// Use the actual due_date from the database instead of calculating it
$history_query = "
SELECT 
              ...
              <?php while ($row = $history_result->fetch_assoc()): ?>
                <div class="bg-white rounded-lg p-5 shadow hover:shadow-md transition">
                      ...
                      <p class="text-xs text-gray-500">by <?= htmlspecialchars($row['author']) ?></p>
                       ...
                       <?php if ($row['status'] === 'overdue'): ?>
                        <p class="text-xs text-red-600 mt-1 font-medium">
                          Penalty: ₱
                          <?php
                          // Calculate penalty: 10 pesos per week overdue (only for overdue books)
                          $due_date = new DateTime($row['due_date']);
                          $current_date = new DateTime();
                          $days_overdue = max(0, $current_date->diff($due_date)->days);
                          $weeks_overdue = ceil($days_overdue / 7); // Round up to charge for partial weeks
                          $penalty = $weeks_overdue * 10;
                  ...
                  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 text-sm">
                    <div>
                      <p class="text-xs text-gray-500">Borrowed Date</p>
                      <p class="font-medium text-gray-700"><?= date('M d, Y', strtotime($row['borrow_date'])) ?></p>
                    </div>
                    <div>
                      <p class="text-xs text-gray-500">Due Date</p>
                      <p class="font-medium text-gray-700"><?= date('M d, Y', strtotime($row['due_date'])) ?></p>
                    </div>
                    <div>
                      <p class="text-xs text-gray-500">Returned Date</p>
                      <p class="font-medium text-gray-700"><?= $row['return_date'] ? date('M d, Y', strtotime($row['return_date'])) : '—' ?></p>
                    </div>
                    <div>
                      <p class="text-xs text-gray-500">Staff</p>
                      <p class="font-medium text-gray-700"><?= htmlspecialchars($row['staff_name'] ?: 'N/A') ?></p>
                    </div>
                  </div>
