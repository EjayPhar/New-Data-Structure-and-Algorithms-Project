...
function restore_user($user_id, $conn) {
    $sql = "UPDATE users SET status = 'active', deleted_at = NULL, deleted_by = NULL WHERE id = ?";
    $stmt = $conn->prepare($sql);
...
// Fetch deleted users - Updated to include deleted_at and deleted_by
$deleted_users_sql = "SELECT id, username, email, role, status, deleted_at, deleted_by FROM users WHERE status = 'disabled' ORDER BY deleted_at DESC";
                                ...
                                <table class="min-w-full divide-y divide-border-light dark:divide-border-dark">
                                    ...
                                    <tbody class="bg-surface-light dark:bg-surface-dark divide-y divide-border-light dark:divide-border-dark">
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap text-sm text-stone-600 dark:text-stone-300"><?php echo htmlspecialchars($book['deleted_at'] ?? 'N/A'); ?></td>
                                            <td class="px-5 py-4 whitespace-nowrap text-sm text-stone-600 dark:text-stone-300"><?php echo htmlspecialchars($book['deleted_by'] ?? 'N/A'); ?></td>
                                ...
                                <table class="min-w-full divide-y divide-border-light dark:divide-border-dark">
                                    ...
                                    <tbody class="bg-surface-light dark:bg-surface-dark divide-y divide-border-light dark:divide-border-dark">
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-stone-600 dark:text-stone-300"><?php echo htmlspecialchars($user['deleted_at'] ?? 'N/A'); ?></td>
                                            <td class="px-7 py-4 whitespace-nowrap text-sm text-stone-600 dark:text-stone-300"><?php echo htmlspecialchars($user['deleted_by'] ?? 'N/A'); ?></td>
                                            <td class="px-4 py-4 whitespace-nowrap">
                                                <div class="flex gap-2">
                                                    <form method="POST" style="display: inline;">
