...
if (!isset($_SESSION['user_id']) || !in_array($_SESSION['role'], ['staff', 'user', 'admin'])) {
    header("Location: ../login/login.html");
    exit();
}

// Determine user role
$is_student = ($_SESSION['role'] === 'user');
...
function getAttendanceRecords($conn, $period = 'week', $onlyUserId = null) {
    ...
    $sql = "SELECT a.id, u.username, u.email, u.role, a.check_in_date, a.check_in_time, a.status
            FROM attendance a
...
// Get data - Students see only their own records
...
$page_title = $is_student ? 'My Attendance' : 'Attendance Logs';
$dashboard_title = $is_student ? 'Student Dashboard' : ($is_staff ? 'Librarian Dashboard' : 'Admin Dashboard');
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title><?php echo $page_title; ?> - Library System</title>
    ...
    <div class="flex h-screen">
        ...
        <aside id="sidebar"
            ...
            <nav class="flex-1 p-4 space-y-2">
                ...
                <a class="flex items-center px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-primary hover:text-white rounded-md transform transition-all duration-150 hover:translate-x-1 hover:shadow-md"
                    href="../user/browse-books.php">
                    <span class="material-icons mr-3">menu_book</span>
                    Browse Books
                </a>
                <a class="flex items-center px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-primary hover:text-white rounded-md transform transition-all duration-150 hover:translate-x-1 hover:shadow-md"
                    href="../user/my-books.php">
        ...
        <div class="flex-1 flex flex-col">
            <header
                ...
                <h1 class="text-xl font-semibold text-slate-800 dark:text-slate-100"><?php echo $dashboard_title; ?></h1>
            ...
            <div class="max-w-6xl mx-auto space-y-8">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <h2 class="text-2xl font-bold text-gray-800 dark:text-white"><?php echo $page_title; ?></h2>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        <div class="flex-1 relative">
                            ...
                            <input id="search-input"
                                ...
                                placeholder="Search by student name or email..." type="text" />
                        </div>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-sm text-primary font-medium mt-1">Showing <span id="record-count"><?php echo count($attendance_records); ?></span> records</p>
                    </div>
                    <div id="attendance-container" class="space-y-4">
                        ...
                        <div
                            class="attendance-record group bg-white dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between hover:shadow-md dark:hover:bg-[#363636] transition-all cursor-pointer"
                            data-username="<?php echo htmlspecialchars(strtolower($record['username'])); ?>"
                            data-email="<?php echo htmlspecialchars(strtolower($record['email'])); ?>">
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    <div class="flex items-center gap-2 mb-1 flex-wrap">
                                        ...
                                        <span
                                            class="text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-0.5 rounded-full border border-blue-200 dark:border-blue-700"><?php echo display_role($record['role']); ?></span>
                                    </div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400 pl-6"><?php echo htmlspecialchars($record['email']); ?></div>
            ...
            if (searchInput) {
                searchInput.addEventListener('input', function(e) {
                    ...
                    records.forEach(record => {
                        const username = record.dataset.username;
